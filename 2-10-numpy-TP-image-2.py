...
l2 = np.random.choice(list(dico), n)
T = patchwork(l2,dico)
plt.imshow(T)
plt.show
...
plt.imshow(im)
#1
tab = np.zeros(shape=(np.shape(im)[0],np.shape(im)[1],np.shape(im)[2]))
tab
...
# 1
def sepia(imrgb) :
    mat = [[0.393,0.769,0.189],[0.349,0.686,0.168],[0.272,0.534,0.131]]
    mat_sepia_rgb = np.array(mat,float)
    res = np.zeros(shape = (np.shape(imrgb)[0],np.shape(imrgb)[1],np.shape(imrgb)[2]))
    for i in range(np.shape(imrgb)[0]) :
        for j in range(np.shape(imrgb)[1]) :
            res[i][j] = np.dot(mat_sepia_rgb,np.array(imrgb[i][j]))
...
# %%
#2
Im = plt.imread('patchwork-all.jpg')
plt.imshow(sepia(Im))
